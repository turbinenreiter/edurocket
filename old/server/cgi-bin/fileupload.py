...
def vis_main(filename):

    path = '/home/oeplse/html'
    filename = filename.split('/')[-1]

    # read file
    data = np.genfromtxt(path+'/data/'+filename,delimiter=',',names=True)
    ...
    plt.tight_layout()
    mpld3.save_html(fig, path+'/output/'+filename.split('.')[0]+'.html')
...
if fileitem.filename:
    ...
    out = open('/home/oeplse/html/data/' + fn, 'wb')
    out.write(fileitem.file.read())

    subprocess.call(['./split.sh'])

    for filename in os.listdir('/home/oeplse/html/data/'):
        try:
            vis_main(filename)
        except:
            pass

    message = 'Go to <a href="http://www.edurocket.org/output/">output for graph</a>'
    out.close()
